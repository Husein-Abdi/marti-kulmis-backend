...
const createHall = async (req, res, next) => {
  try {
    const { name, location, capacity,price,description,hallCreater } = req.body;

    if (!name || !location || !capacity || !price || !description || !hallCreater) {
    ...
    const hall = new Hall({ name, location, capacity,price,description,hallCreater });
    await hall.save();
...
const updateHall = async (req, res, next) => {
  try {
    ...
    const updatedHall = await Hall.findByIdAndUpdate(
      hallId,
      { name, location, capacity, price, description },
      { new: true }
    );
