...
const generateBookingEmailTemplate = (eventName, bookedHallName, bookingId) => {
  return `
...
const createBooking = async (req, res, next) => {
  try {
    ...
    if (!eventManager || !phoneNumber || !eventName ) {
      return res.status(422).json({ error: "Please fill all details" });
   ...
   if (endDateTime <= startDateTime) {
    ...
    const mailOptions = {
      ...
      html:   generateBookingEmailTemplate(eventName, bookedHallName, booking._id),
...
const updateBooking = async (req, res, next) => {
  try {
    ...
    const sendApprovalEmail = async (booking, bookingId) => {
      try {
        ...
        const mailOptions = {
          ...
          html: sendApprovalEmailTemplate(booking.eventName, booking.bookedHallName, bookingId),
        };
    ...
    const sendRejectionEmail = async (booking,  bookingId ,rejectionReason) => {
      try {
        ...
        const mailOptions = {
          ...
          html: sendRejectionEmailTemplate(booking.eventName, booking.bookedHallName, bookingId ,rejectionReason),
        };
    ...
    const sendRejectionEmailTemplate = (eventName, bookedHallName, bookingId ,rejectionReason) => {
      return `
    ...
    const sendApprovalEmailTemplate = (eventName, bookedHallName, bookingId) => {
      return `
